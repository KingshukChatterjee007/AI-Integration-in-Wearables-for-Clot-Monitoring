...
def train_v6_augmented():
    ...
    # Aggressive weights: EMERGENCY (2) = 10.0, WARNING (1) = 3.0, SAFE (0) = 1.0
    ...
    for epoch in range(epochs):
        ...
        # Validation on UNSEEN Subjects 9 & 10
        model.eval()
        correct = 0
        total = 0
        with torch.no_grad():
        ...
        if val_acc > best_acc:
            best_acc = val_acc
            torch.save(model.state_dict(), "trained_models/clot_hybrid_v6_augmented.pth")
            
    logger.info(f"V6 Augmented Training Complete. Best Val Accuracy: {best_acc*100:.2f}%")
